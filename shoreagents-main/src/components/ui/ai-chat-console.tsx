...
import { MayaTextField, MayaNameFields, MayaAnonymousUserForm, MayaTalentSearchModal } from '@/components/maya';
...
const ChatConsole: React.FC<ChatConsoleProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { response, relatedContent, userData } = await generateAIResponse(inputValue, messages);
      ...
      addMessage(assistantMessage);
       ...
       // Check if Maya is suggesting talent search
       const isSuggestingTalentSearch = responseLower.includes('talent_search_modal') ||
                                       responseLower.includes('let me show you our talent pool') ||
                                       responseLower.includes('browse our talent pool') ||
                                       responseLower.includes('show you our talent') ||
                                       responseLower.includes('talent pool') ||
                                       (responseLower.includes('talent') && responseLower.includes('pool'));
       
       if (isSuggestingTalentSearch && !isTalentSearchOpen) {
         console.log('Maya is suggesting talent search, opening modal');
         setIsTalentSearchOpen(true);
       }
    } catch (error) {
